...
"""def separar_palabras(self):
    #texto = self.textEdit.toPlainText() + ' $'  # Con espacio para que lea bien el split
    #self.textEdit_2.setPlainText(texto)
    #texto_spliteado = texto.split()
    analizado = self.lexemaAnalizador()  # Analizamos el texto
    for i in range(len(analizado), self.tableWidget.rowCount()):  # Eliminamos las filas
        self.tableWidget.removeRow(len(analizado))

    #for r in range(len(texto_spliteado)):  # Populamos
    for r in range(len(analizado)):  # Populamos
        rowAct = self.tableWidget.rowCount()
        if rowAct <= r:  # Control del num de filas
            self.tableWidget.insertRow(rowAct)
            #self.tableWidget.setItem(r, 0, QtWidgets.QTableWidgetItem(texto_spliteado[r]))
            #self.tableWidget.setItem(r, 0, QtWidgets.QTableWidgetItem(analizado[r]['lexema']))
            #self.tableWidget.setItem(r, 1, QtWidgets.QTableWidgetItem(analizado[r]['token']))
            #self.tableWidget.setItem(r, 2, QtWidgets.QTableWidgetItem(str(tokenTraductor(analizado[r]['token']))))
        else:
            #self.tableWidget.setItem(r, 0, QtWidgets.QTableWidgetItem(texto_spliteado[r]))
            #self.tableWidget.setItem(r, 0, QtWidgets.QTableWidgetItem(analizado[r]['lexema']))
            #self.tableWidget.setItem(r, 1, QtWidgets.QTableWidgetItem(analizado[r]['token']))
